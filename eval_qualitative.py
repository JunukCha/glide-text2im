...
qualitative_texts = [
    "The blue light of the computer monitor", 
    "Happy party at night", 
    "Roaring and crackling fire", 
    "Eerie glow of green fluorescent lighting", 
    "The chilling atmosphere of the fear-inducing dark room", 
]
...
def main():
    ...
    source_list = glob.glob("../../qualitative/*.jpg")
    mask_list = glob.glob("../../qualitative/*_mask.png")
    source_list.sort()
    mask_list.sort()
    ...
    for text_index in range(len(qualitative_texts)):
        ...
        for index, (source_path, mask_path) in \
            ...
            reshaped_pil.save(osp.join(results_folder, f"{text_index}_pred.jpg"))
            
            concatenated_image = concatenate_horizontally_pil([source_orig, reshaped_pil], padding_size=10)
            concatenated_image.save(os.path.join(results_folder, f"{text_index}_concatenated_image.jpg"))
            
            with open(os.path.join(results_folder, f"{text_index}_text.txt"), "w") as f:
                f.write(prompt)
